...
df1 <- df |> 
  group_by(size_flag) |> 
  summarise(
    mean_ed_score = mean(education_score, na.rm = T), na.rm = T
  ) |> 
...
ggplot(data = df, aes(x = education_score, y = size_flag), ) +
  geom_dots(smooth = smooth_discrete(kernel = "biweight"), stackratio = 0.8,
    side = "both", layout = "swarm", slab_shape = 21, width = 0.8,
                            slab_color = "#27A0CC", slab_fill = "#27A0CC",
                    scale = 0.65, overflow = "compress", binwidth = unit(c(1.6, Inf), "mm")) +
  geom_text(aes(x = -11, y = size_flag, label = size_flag), color = "grey50", 
  ...
  annotate("segment", x = df1$mean_ed_score[1], xend = df1$mean_ed_score[1], y = 1.6, yend = 2.4, linewidth = 0.8) +
    annotate("segment", x = df1$mean_ed_score[2], xend = df1$mean_ed_score[2], y = 2.6, yend = 3.4, linewidth = 0.8) +
      annotate("segment", x = df1$mean_ed_score[3], xend = df1$mean_ed_score[3], y = 3.6, yend = 4.4, linewidth = 0.8) +
      annotate("segment", x = df1$mean_ed_score[4], xend = df1$mean_ed_score[4], y = 4.6, yend = 5.4, linewidth = 0.8) +
