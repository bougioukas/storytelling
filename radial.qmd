...
ggplot() +
  ...
  geom_text(data = filter(df, group == "group1"), 
            aes(x = 0, y, label = country), size = 6, hjust = 1.4) +
  scale_color_manual(values = cols1) +
  guides(color = guide_legend(title = paste0(strrep(" ", 7), "After 2004"),
                              override.aes = list(size = c(5, 5, 5)),
                              reverse = TRUE,
                              keywidth = 1.5, 
  ...
  scale_x_continuous(limits = c(0, 20)) +  
  scale_y_continuous(expand = expansion(mult = 0.3), limits = c(0, 3)) +
  guides(color = guide_legend(title = paste0(strrep(" ", 7), "Before 2004", strrep(" ", 10)),
                              reverse = TRUE, override.aes = list(size = c(5, 5, 5)),
